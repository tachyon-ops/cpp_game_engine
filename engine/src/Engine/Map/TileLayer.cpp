  ...
  for (unsigned int i = 0; i < m_RowCount; i++) {
    for (unsigned int j = 0; j < m_ColCount; j++) {
      ...
      if (tileID == 0) {
        ...
        //     ts.Name, ts.TileSize, j * ts.TileSize - cam.X,
        //     i * ts.TileSize - cam.Y, tileRow, tileCol);
        TextureManager::GetInstance()->DrawTile(
            ts.Name, ts.TileSize, j * ts.TileSize, i * ts.TileSize, tileRow,
            tileCol);
      }
    }
  }
